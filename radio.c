...
void RADIO_ConfigureSquelchAndOutputPower(VFO_Info_t *pInfo)
{
	...
	// Fine squelch: level = SQUELCH_LEVEL + SQUELCH_TENTHS/10, 0.0 .. 9.0.
	// The calibration tables only hold whole levels 1..9, so the thresholds
	// for the tenths are interpolated between the two neighbouring whole
	// levels (level 0 = wide open). That gives 90 usable steps, most of the
	// useful ones being between 0 and 1 where the stock firmware has nothing.
	{
		const uint8_t lvl10 = (uint8_t)(gEeprom.SQUELCH_LEVEL * 10u + gEeprom.SQUELCH_TENTHS);
		const uint8_t lo    = lvl10 / 10u;
		const uint8_t frac  = lvl10 % 10u;
		const uint8_t hi    = (lo < 9 && frac) ? lo + 1 : lo;
		uint8_t tlo[6], thi[6];   // rssi_open, rssi_close, noise_open, noise_close, glitch_close, glitch_open

		static const uint8_t open_thresholds[6] = { 0, 0, 127, 127, 255, 255 };

		for (unsigned k = 0; k < 2; k++) {
			const uint8_t level = k ? hi : lo;
			uint8_t *t = k ? thi : tlo;
			if (level == 0) {
				memcpy(t, open_thresholds, 6);
			} else {
				for (unsigned i = 0; i < 6; i++)
					EEPROM_ReadBuffer(Base + level + i * 0x10, &t[i], 1);   // VHF 50/40/65/70/90/100, UHF 10/5/90/100/90/100
			}
		}

		uint8_t th[6];
		for (unsigned i = 0; i < 6; i++)
			th[i] = (uint8_t)(((uint16_t)tlo[i] * (10u - frac) + (uint16_t)thi[i] * frac + 5u) / 10u);

		pInfo->SquelchOpenRSSIThresh    = th[0];
		pInfo->SquelchCloseRSSIThresh   = th[1];
		pInfo->SquelchOpenNoiseThresh   = th[2];
		pInfo->SquelchCloseNoiseThresh  = th[3];
		pInfo->SquelchCloseGlitchThresh = th[4];
		pInfo->SquelchOpenGlitchThresh  = th[5];
	}

	if (gEeprom.SQUELCH_LEVEL != 0 || gEeprom.SQUELCH_TENTHS != 0)
	{
