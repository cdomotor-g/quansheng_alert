...
void SETTINGS_InitEEPROM(void)
{
	...
	// 0F48..0F4F  quansheng_alert: fine squelch + ALERT app config
	EEPROM_ReadBuffer(0x0F48, Data, 8);
	gEeprom.SQUELCH_TENTHS  = (Data[0] < 10) ? Data[0] : 0;
	gEeprom.SQL_OPEN_DELAY  = ((Data[1] & 7u) < 8 && Data[1] != 0xFF) ? (Data[1] & 7u) : 1;
	gEeprom.SQL_CLOSE_DELAY = (Data[1] != 0xFF) ? ((Data[1] >> 3) & 3u) : 2;
	memcpy(gEeprom.ALERT_CFG, &Data[2], sizeof(gEeprom.ALERT_CFG));
